...
export default {
  ...
  methods: {
    ...
    handleClick(tab) {
      ...
      if (tab.name === "six") {
        ...
        if (this.$router.history.current.path !== "/des") {
    ...
    confirmCity(city) {
      ...
      sessionStorage.cityName = this.cityName;
       // 保存城市
      this.$store.dispatch('setCityFunc', this.cityName)
      // 获取城市链接
      this.GetCityFileLinkAsync();
      // 获取天气
      this.GetWeatherAsync();
  ...
  mounted() {
    ...
    if (this.activeName === "six" && this.$router.history.current.path !== "/Vaccine/livedata") {
      this.$router.push("/Vaccine");
    }
  },
  updated() {
